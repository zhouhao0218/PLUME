...
class PlumeAB_resid(nn.Module):

    def __init__(self, base_linear: nn.Linear, A, B, r, k=16, resid_rank: int = 1):
        super().__init__()
    ...
    def inject_Cu(self):
        ...
        self.Au = nn.Parameter(torch.zeros(out_features, self.resid_rank, device=device, requires_grad=True))
        self.Bu = nn.Parameter(torch.empty(in_features, self.resid_rank, device=device, requires_grad=True))
    ...
    def forward(self, x):
        if self.training or self._cached_W_user is None:
            if self.phase == "full":
                ...
                W_low = self.B @ (I + 2 * self.Cu) @ self.A   # (out, r) @ (r, r) @ (r, in) -> (out, in)
                Bu_norm = torch.norm(self.Bu, p='fro') + 1e-8
                W_resid = (self.Au) @ (self.Bu.T)
...
class CustomArgs(TrainingArguments):
    ...
    resid_rank: int = field(default=1, metadata={"help": "Residual rank for Au/Bu in PlumeAB_resid"})
...
def replace_with_plume_from_peft(peft_model, r, svd_cache_dir=None, resid_rank: int = 1):
    to_replace = []
    ...
    for name, module in to_replace:
        ...
        plumer = PLUME_LAYER_CLS(base_linear, A, B, r=r, resid_rank=resid_rank)
...
def main():
    ...
    replace_with_plume_from_peft(model, r=args.lora_r, svd_cache_dir=svd_cache_dir, resid_rank=args.resid_rank)
        ...
        if args.plume_mode in ["phase1", "phase2", "phase3", "full"]:
            if not load_aplaud_params(user['user_id'], model, args, phase="phase1"):
                for module in model.modules():
                    if isinstance(module, PLUME_LAYER_CLS):
                        module.inject_Cu()
                # Print trainable modules and parameters
                if user['user_id'] == user_data_list[0]['user_id']:  # Only print for the first user
                    print(f"🔧 Training Phase 1 for user {user['user_id']}")
                    print("Trainable modules and parameters:")
