...
class TrainingArguments(transformers.TrainingArguments):
    ...
    adapter_name_or_path: Optional[str] = field(
        default=None,
        metadata={"help": ("Pre-initialized PiSSA adapter path; when this is not None, the following arguments are ignored.")},
    )
    init_weights: bool | str = field(
        default=True,
        metadata={"help": ("True -> LoRA; `pissa` -> PiSSA; `pissa_niter_16` -> Fast SVD PiSSA")},
    )
    ...
    lora_dropout: Optional[float] = field(default=0.0, metadata={"help": ("Must be set to 0 when using PiSSA.")})
    ...
    # DataArguments:
    data_path: str = field(default=None, metadata={"help": "Path to the training data. Can be a JSON/JSONL file or a directory."})
    sub_task: List[str] = field(default_factory=lambda: ["main"])
...
def train_tokenize_function(examples, tokenizer, query, response):
    ...
    targets = [
        (str(y) + tokenizer.eos_token) if not str(y).endswith(str(tokenizer.eos_token)) else str(y)
        for y in examples[response]
    ]
...
def build_model(script_args, checkpoint_dir):
    ...
    if not script_args.full_finetune:
        ...
        if checkpoint_dir is not None:
            ...
            print(f"Initialize LoRA/PiSSA/CLOVER adapters from {script_args.model_name_or_path}/{script_args.adapter_name_or_path}.")
            ...
            print(f'Init LoRA/PiSSA modules...')
        ...
        # Ensure LoRA adapters are trainable - only print on rank 0
...
def train():
    ...
    print("Load tokenizer from {} over.".format(script_args.model_name_or_path))
    ...
    for task in script_args.sub_task:
        ...
        # ===== Local JSON/JSONL support =====
        if script_args.data_path is not None and os.path.exists(script_args.data_path):
            if os.path.isfile(script_args.data_path):
                ext = os.path.splitext(script_args.data_path)[1].lower()
                assert ext in [".json", ".jsonl"], f"Unsupported file type: {ext}"
                ds_all = load_dataset("json", data_files={script_args.dataset_split: script_args.data_path})
                ds = ds_all[script_args.dataset_split]
            else:
                # directory: default load all *.jsonl
                pattern = os.path.join(script_args.data_path, "*.jsonl")
                files = sorted(glob.glob(pattern))
                assert len(files) > 0, f"No jsonl files found under {script_args.data_path}"
                ds_all = load_dataset("json", data_files={script_args.dataset_split: files})
                ds = ds_all[script_args.dataset_split]

            # simple slicing like train[:K]
            if cur_split != script_args.dataset_split and cur_split.startswith(f"{script_args.dataset_split}[:"):
                k = int(cur_split.split("[:")[-1].rstrip("]"))
                ds = ds.select(range(min(k, len(ds))))
        else:
            # original HF dataset path usage (kept for compatibility)
            ds = load_dataset(script_args.data_path, data_dir=cur_task, split=cur_split)

        if script_args.local_rank == 0:
            print(f"Loaded {len(ds)} rows for task={cur_task}, split={cur_split}")
    ...
    query_field, resp_field = script_args.dataset_field[0], script_args.dataset_field[1]
    ...
    train_dataset = raw_train_datasets.map(
        ...
        fn_kwargs={"tokenizer": tokenizer, "query": query_field, "response": resp_field}
    )
    print("end map")

    # Both ranks hit the barrier
    ...
    if script_args.local_rank == 0:
        ...
        for index in random.sample(range(min(len(train_dataset), 3)), k=min(3, len(train_dataset))):
            print(f"Sample {index} input_ids: {train_dataset[index]['input_ids'][:64]} ...")
            print(f"Sample {index} text: {tokenizer.decode(list(train_dataset[index]['input_ids']))[:256]} ...")
    ...
    if not script_args.full_finetune and script_args.merge:
        ...
        model.save_pretrained(os.path.join(script_args.output_dir, 'ft_merged'))
        tokenizer.save_pretrained(os.path.join(script_args.output_dir, 'ft_merged'))
